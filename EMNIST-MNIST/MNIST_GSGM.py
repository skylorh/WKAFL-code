...
Gradients_pa = ZerosGradients(Layers_shape, device)
Gradients_Momentum = ZerosGradients(Layers_shape, device)
Gradients_unbiased = ZerosGradients(Layers_shape, device)
...
for itr in range(1, args.total_iterations + 1):
    ...
    # step 2
    # 所有的用户都训练完一轮时更新Gradients_Momentum 并 清空worker_have_list，开启下一轮训练
    ...
    for grad_idx, params_server in enumerate(model.parameters()):
        params_server.data.add_(-args.lr, Collect_Gradients[grad_idx])
    ...
    for worker_idx in range(len(workers_list)):
        ...
        # if worker_idx == 0:
        #     print('worker_model.parameters().data before: ')
        #     for before_idx, before_value in enumerate(worker_model.parameters()):
        #         if before_idx == 0:
        #             print(before_value.data)
        ...
        # if worker_idx == 0:
        #     print('worker_model.parameters().data after: ')
        #     for after_idx, after_value in enumerate(models[workers_list[worker_idx]].parameters()):
        #         if after_idx == 0:
        #             print(after_value.data)
