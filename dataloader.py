...
class FederatedDataLoader(object):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for worker in self.workers:
            if batch_size >=1:
                batch = batch_size
            elif batch_size < 1 and batch_size > 0:
                batch = math.ceil(batch_size * len(federated_dataset[worker]))

            #print(len(federated_dataset[worker]),batch)
            ...
            if batch_num == 0:
                ...
                batch_sampler = BatchSampler(sampler, batch, drop_last)
            elif batch_num >= 1:
                batch_sampler = []
                for i in range(0, batch_num):
                    data_range = range(len(federated_dataset[worker]))
                    sample = random.sample(data_range, batch)
                    batch_sampler.append(sample)
    ...
    def __len__(self):
        print(len(self.federated_dataset))
